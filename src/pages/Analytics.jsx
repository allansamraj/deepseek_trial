  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <select
            ...
            className="px-3 py-1.5 rounded-lg text-sm bg-surface-3 text-[#A1A1AA] border border-[rgba(30,62,98,0.5)] focus:border-[#FF6500] outline-none"
          >
          ...
          <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-surface-3 text-[#A1A1AA] border border-[rgba(30,62,98,0.5)] hover:border-white transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { icon: Search, label: 'Search Queries', value: '45,231', trend: '+28.3%', color: '#FF6500' },
          { icon: TrendingUp, label: 'Average Accuracy', value: '98.6%', trend: '+2.1%', color: '#FF6500' },
          { icon: Users, label: 'Active Users', value: '342', trend: '+15.7%', color: '#1E3E62' },
          { icon: FileText, label: 'New Documents', value: '175', trend: '+12.5%', color: '#FF6500' }
        ].map((stat, i) => {
          ...
          return (
            ...
            >
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-xs font-semibold text-[#FF6500] bg-[rgba(255,101,0,0.1)] px-2 py-0.5 rounded-full">{stat.trend}</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-6" style={{ background: 'rgba(11, 25, 44, 0.6)', border: '1px solid rgba(30, 62, 98, 0.5)' }}>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyActivity} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(30, 62, 98, 0.1)" />
                <XAxis dataKey="name" stroke="#71717A" fontSize={10} tickLine={false} />
                <YAxis stroke="#71717A" fontSize={10} tickLine={false} />
                <Tooltip
                  contentStyle={{ background: 'rgba(11, 25, 44, 0.95)', border: '1px solid rgba(30, 62, 98, 0.5)', borderRadius: '12px' }}
                  labelStyle={{ color: '#F4F4F5', fontWeight: 'bold' }}
                />
                <Area type="monotone" dataKey="queries" stroke="#FF6500" strokeWidth={2} fillOpacity={1} fill="url(#queriesColor)" name="Queries" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={departmentStats} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="documents" fill="#1E3E62" radius={[4, 4, 0, 0]} name="Documents" />
                <Bar dataKey="queries" fill="#FF6500" radius={[4, 4, 0, 0]} name="Queries Activity" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Top Queries Table */}
        <div className="lg:col-span-2 rounded-2xl p-6" style={{ background: 'rgba(11, 25, 44, 0.6)', border: '1px solid rgba(30, 62, 98, 0.5)' }}>
          <h2 className="text-sm font-bold text-[#F4F4F5] uppercase tracking-wider mb-4">Top AI Search Queries</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-[rgba(30,62,98,0.3)]">
              ...
              <tbody>
                {topQueries.map((q, i) => (
                  <tr key={i} className="border-b border-[rgba(30,62,98,0.15)] last:border-0">
                    ...
                    <td className="py-3.5 text-right text-[#FF6500] font-semibold">{q.success}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-4">
            {[
              ...
              <div
                ...
                style={{
                  background: alert.level === 'high' ? 'rgba(244, 63, 94, 0.05)' : alert.level === 'medium' ? 'rgba(245, 158, 11, 0.05)' : 'rgba(255, 101, 0, 0.05)',
                  border: `1px solid ${alert.level === 'high' ? 'rgba(244, 63, 94, 0.2)' : alert.level === 'medium' ? 'rgba(245, 158, 11, 0.2)' : 'rgba(255, 101, 0, 0.2)'}`
                }}
              >
                <AlertCircle size={16} className={alert.level === 'high' ? 'text-[#F43F5E]' : alert.level === 'medium' ? 'text-[#FF6500]' : 'text-[#FF6500]'} />
